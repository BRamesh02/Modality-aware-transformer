...
from src.evaluation.portfolio.robustness import (
    bootstrap_analysis,
...
CANONICAL_PRED_PATH = (
    PROJECT_ROOT / "data/processed/predictions/canonical_walkforward.parquet"
)
...
def main():
    ...
    if "return" in df_returns.columns:
        ...
        raise ValueError(
            f"Could not find return column. Available: {df_returns.columns}"
        )

    if "permno" in df_returns.columns:
        returns_matrix = df_returns.pivot(
            index="date", columns="permno", values=ret_col
        ).fillna(0.0)
    else:
    ...
    if CANONICAL_PRED_PATH.exists():
        models["Canonical"] = SignalFactory(
            pd.read_parquet(CANONICAL_PRED_PATH)
        ).get_all_signals()
    ...
    for model_name, signals in models.items():
        ...
        for strat_name, signal_df in signals.items():
            bt = run_backtest(
                signal_df, returns_matrix, cost_bps=COST_BPS, universe_mask=df_universe
            )
    ...
    pd.DataFrame(results).to_csv(
        OUTPUT_DIR / "tables" / "all_strategies_metrics.csv", index=False
    )
    ...
    for model_name, (best_strat, _) in best_strategies.items():
        ...
        bt = run_backtest(
            signal, returns_matrix, cost_bps=COST_BPS, universe_mask=df_universe
        )
        ...
        if df_factors is not None:
            try:
                ...
                attr_summary = attr_summary.reset_index().rename(
        ...
        autopsy_list.append(
            {
                ...
                "Strategy": best_strat,
                "Leg": "Long",
                "Return": long_ret.mean() * 252,
                "Vol": long_ret.std() * np.sqrt(252),
            }
        )
        autopsy_list.append(
            {
                ...
                "Strategy": best_strat,
                "Leg": "Short",
                "Return": short_ret.mean() * 252,
                "Vol": short_ret.std() * np.sqrt(252),
            }
        )

    if attribution_list:
        pd.concat(attribution_list).to_csv(
            OUTPUT_DIR / "tables" / "champion_attribution.csv", index=False
        )

    if autopsy_list:
        pd.DataFrame(autopsy_list).to_csv(
            OUTPUT_DIR / "tables" / "champion_autopsy.csv", index=False
        )
