...
def run_pure_inference(df_main:pd.DataFrame, model_type:str, config:dict):
    ...
    for split in target_splits:
        ...
        _, _, df_test = prepare_scaled_fold(df_main, scale_cols, split, buffer_days=math.ceil(1.5*config["window_size"]))
...
def main():
    print("Loading Data...")
    df_main = load_and_merge_data(PROJECT_ROOT / "data", start_date=CONFIG["start_date"], end_date=CONFIG["end_date"])
