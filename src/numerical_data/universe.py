...
def parse_wrds_legacy_csvs(
    folder_path: Path, start_year: int, end_year: int
) -> pd.DataFrame:
    """
...
def process_github_presence(
    file_path: Path, wrds_client: WRDSClient, start_date: str
) -> pd.DataFrame:
    """
    ...
    df_exploded["clean_ticker"] = (
        df_exploded["ticker"].map(TICKER_CLEANING_MAP).fillna(df_exploded["ticker"])
    )
    df_exploded = df_exploded.dropna(subset=["clean_ticker"])

    unique_tickers = df_exploded["clean_ticker"].unique().tolist()
    df_ids = wrds_client.get_ticker_to_permno_mapping(
        unique_tickers, active_after="2015-01-01"
    )
    ...
    valid_links = merged[
        (merged["date"] >= merged["namedt"]) & (merged["date"] <= merged["nameenddt"])
    ].copy()
...
def fuse_presence_matrices(
    df_wrds: pd.DataFrame, df_wiki: pd.DataFrame, split_date: str
) -> pd.DataFrame:
    """
    ...
    print(
        f"Timeline: {df_combined.index.min().date()} to {df_combined.index.max().date()}"
    )
...
def build_full_presence_matrix(
    legacy_dir: Path, github_file: Path, wrds_client: WRDSClient
) -> pd.DataFrame:
    """
