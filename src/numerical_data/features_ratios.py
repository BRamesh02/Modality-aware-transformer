...
def clean_and_merge_ratios(
    df_daily_skeleton: pd.DataFrame, df_ccm: pd.DataFrame, df_ibes: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    df_ccm = df_ccm.sort_values("public_date").drop_duplicates(
        subset=["permno", "public_date"], keep="last"
    )
    df_ibes = df_ibes.sort_values("public_date").drop_duplicates(
        subset=["permno", "public_date"], keep="last"
    )
    ...
    drop_cols = [
        ...
        if "_ibes" in c or "public_date" in c or "_drop" in c
    ]
