...
def process_monthly_macro(
    s_unrate: pd.Series, s_cpi: pd.Series, s_ppi: pd.Series, start_date: str
) -> pd.DataFrame:
    """
...
def process_daily_macro(
    s_yield_curve: pd.Series, s_risk_free: pd.Series, s_vix: pd.Series
) -> pd.DataFrame:
    """
...
def merge_and_format_macro(
    df_monthly: pd.DataFrame, df_daily: pd.DataFrame
) -> pd.DataFrame:
    """
    Fuses the two streams and applies 'macro_' namespace.
    """
    df_merged = pd.merge(
        left=df_monthly, right=df_daily, left_index=True, right_index=True, how="outer"
    )
