...
def compute_quantile_returns(
    ...
    buckets = (ranks * n_bins).apply(np.floor).fillna(-1).astype(int).clip(upper=n_bins - 1)
...
def plot_quintiles_scientific(df_quantiles: pd.DataFrame, title: str, save_path):
    ...
    for i, col in enumerate(cum_ret.columns):
        ...
        label_suffix = " (Top)" if i == 0 else (" (Bottom)" if i == len(cum_ret.columns)-1 else "")
        ax.plot(cum_ret[col], color=colors[i], linewidth=lw, alpha=alpha, label=f"{col}{label_suffix}")
