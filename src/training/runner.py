...
def train_model_for_year(
    ...
    early_stopping = EarlyStopping(
        patience=model_config["patience"], path=str(save_path), verbose=False
    )
    ...
    for epoch in range(EPOCHS):
        t_loss = train_epoch(
            model, model_config, train_loader, optimizer, criterion, device, scheduler
        )
        v_loss, v_ic = validate_epoch(
            model, model_config, val_loader, criterion, device
        )

        current_lr = scheduler.get_last_lr()[0]
        print(
            f"   Epoch {epoch+1}/{EPOCHS} | Train Loss: {t_loss:.6f} | Val Loss: {v_loss:.6f} | Val IC: {v_ic:.4f} | LR: {current_lr:.2e}"
        )
        ...
        if early_stopping.early_stop:
            print(
                f"      [Early Stopping] Epoch {epoch+1} - Best Val IC: {early_stopping.best_ic:.4f}"
            )
            break
...
def run_walk_forward(
    df_main: pd.DataFrame, model_type: str, config: dict, project_root: Path
):
    ...
    if not target_splits:
        print(
            f"No splits found for range {start_year}-{end_year}. Check your data dates."
        )
        return pd.DataFrame()
    ...
    for split in target_splits:
        ...
        train_ds = FinancialDataset(
            ...
            window_size=ws,
            forecast_horizon=fh,
            min_date=split["train"][0],
            max_date=split["train"][1],
        ...
        val_ds = FinancialDataset(
            ...
            window_size=ws,
            forecast_horizon=fh,
            min_date=split["val"][0],
            max_date=split["val"][1],
            use_emb=config["use_emb"],
        )
        ...
        best_weights_path = train_model_for_year(
            year, train_loader, val_loader, model_type, config, device, models_dir
        )
